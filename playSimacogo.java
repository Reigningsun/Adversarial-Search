...
public class playSimacogo {
			...
			if (board[4][column] == 9){																// Open space is between 1-4 search index 1-4 
				for (short row = 1; row < 5; row++){
					if (board[row][column] == 9){													// Generate move in first empty space
						...
						Simacogo2 move = new Simacogo2(modified, gameScore, 
	...
	public static short alphaBeta(Simacogo2 gameState, byte depth, double alpha, 							
								  double beta, boolean aiPlayer){
		if (depth <= 0){
			...
			if (aiPlayer == true){
				while (moves.size() > 0){
					Simacogo2 move = moves.pop();
					alpha = Math.max(alpha, alphaBeta(move, (byte) (depth -1), 						// Choose the best option the minimizing player would allow
							                          alpha, beta, false)); 				
				...
				while (moves.size() > 0){
					Simacogo2 move = moves.pop();
					beta = Math.min(beta, alphaBeta(move, (byte) (depth -1), 						// Choose the best option the maximizing player would allow
													alpha, beta, true));
